...
def get_companies(
    status: Optional[str] = Query(None, description="Filter by status (e.g. pending_review, enriched, rejected)"),
    approval_status: Optional[str] = Query(None, description="Filter by approval_status (pending, approved, rejected)"),
    ...
    if status:
        ...
        query = query.filter(Company.status.in_(statuses))
